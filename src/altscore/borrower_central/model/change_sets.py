...
from altscore.common.http_errors import (
    raise_for_status_improved,
    retry_on_401,
    retry_on_401_async,
)
from altscore.borrower_central.model.generics import (
    GenericSyncResource,
    GenericAsyncResource,
    GenericSyncModule,
    GenericAsyncModule,
)
...
class CreateChangeItemDTO(BaseModel):
    ...
    entity_id: str = Field(alias="entityId")
    suggested_values: Dict[str, Any] = Field(alias="suggestedValues")
    overridden_values: Optional[Dict[str, Any]] = Field(
        alias="overriddenValues", default=None
    )
...
class ChangeSetSyncModule(GenericSyncModule):
    ...
    @retry_on_401
    def run_operation(
        self, change_set_id: str, dto: RunOperationRequest
    ...
    @retry_on_401
    def get_summary_by_risk_rating(
        self, change_set_id: str
    ) -> List[ChangeSetSummaryByRiskRatingDTO]:
        ...
        with httpx.Client(
            ...
            raise_for_status_improved(response)
            return [
                ChangeSetSummaryByRiskRatingDTO.parse_obj(item)
...
class ChangeSetAsyncModule(GenericAsyncModule):
    ...
    @retry_on_401_async
    async def get_summary_by_risk_rating(
        self, change_set_id: str
    ) -> List[ChangeSetSummaryByRiskRatingDTO]:
        """Get summary of preapproval change items grouped by risk rating"""
        async with httpx.AsyncClient(
